...
__constant__ uint8_t low_threshold = 4;
__constant__ uint8_t high_threshold = 30;
...
__device__ bool is_strong_edge(int x, int y, bool *strongEdges, int width, int height)
{
...
extern "C"
{
    __global__ void compute_min_values_kernel(uchar3 *buffer, uchar3 *buffer_cpy,
                                              int width, int height)
    {
        ...
        if (x < width && y < height)
        {
            ...
            buffer[index].x = min(255, static_cast<int>(buffer[index].x + 0.5 * buffer_cpy[index].x));
        }
    }

    __global__ void apply_color_to_buffer_kernel(uchar3 *d_buffer_cpy, bool *d_strong_edges,
                                                 int width, int height)
    {
        int x = threadIdx.x + blockIdx.x * blockDim.x;
        int y = threadIdx.y + blockIdx.y * blockDim.y;
        ...
        if (x < width && y < height)
        {
            ...
            if (d_strong_edges[index])
            {
                // If strong edge, set the pixel to white
                d_buffer_cpy[index] = make_uchar3(255, 255, 255); // White
            }
            else
            {
                // If not a strong edge, set the pixel to black
                d_buffer_cpy[index] = make_uchar3(0, 0, 0); // Black
            }
        }
    }

    __global__ void hysteresis_thresholding_kernel(uchar3 *image, bool *strongEdges, bool *weakEdges,
                                                   int width, int height)
    {
        int x = threadIdx.x + blockIdx.x * blockDim.x;
        int y = threadIdx.y + blockIdx.y * blockDim.y;
        ...
        if (x < width && y < height)
        {
            ...
            // Convert uchar3 to grayscale (you might need to adjust this based on your image format)
            // uint8_t intensity = static_cast<uint8_t>((static_cast<int>(pixel.x) + static_cast<int>(pixel.y) + static_cast<int>(pixel.z)) / 3);
            uint8_t intensity = static_cast<uint8_t>(pixel.x);
            ...
            // Wait for all threads to finish writing to weakEdges before proceeding
            __syncthreads();
                ...
                for (int dy = -1; dy <= 1; ++dy)
                {
                    for (int dx = -1; dx <= 1; ++dx)
                    {
                        ...
                        if (neighborX >= 0 && neighborX < width && neighborY >= 0 && neighborY < height &&
                            is_strong_edge(neighborX, neighborY, strongEdges, width, height))
                        {
                            strongEdges[index] = true;
                            break;
                        }
