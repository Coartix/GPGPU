...
extern "C"
{
    ...
    */
                      ...
                      int width, int height, int stride, int pixel_stride)
    {
        ...
        erosionKernel<<<gridSize, blockSize>>>(d_input, d_output, width, height, stride, pixel_stride);
        dilationKernel<<<gridSize, blockSize>>>(d_output, d_input, width, height, stride, pixel_stride);
        err = cudaMemcpy(d_buffer_cpy, d_input, size, cudaMemcpyDeviceToDevice);
        CHECK_CUDA_ERROR(err);
