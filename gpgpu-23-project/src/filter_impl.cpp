...
extern "C" {
    ...
    */
     ...
     * Copy buffer to temporary first to avoid propagation
    */
    void apply_dilation(uint8_t* buffer, int width, int height, int stride,
            int pixel_stride) {
        // Temporary buffer to store the results
        uint8_t* temp = new uint8_t[width * height * pixel_stride];
        std::memcpy(temp, buffer, width * height * pixel_stride);
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                ...
                temp[y * stride + x * pixel_stride] = max_val;
            }
        }
            ...
            for (int x = 0; x < width; ++x) {
                ...
                if (mask[y * stride + x * pixel_stride] == 0) {
                    for (int channel = 0; channel < 3; ++channel) {
                        ...
                        back_ground_model[index] = (frame_count * back_ground_model[index] + current_frame[index]) / (frame_count + 1);
                    }
                }
            }
        }
    ...
    void filter_impl(uint8_t* buffer, int width, int height, int stride,
            int pixel_stride) {
        const int n = 20; // Number of frames to keep in history

         // Check if the background model is initialized
        ...
        //std::memcpy(buffer, back_ground_model, width * height * pixel_stride);
